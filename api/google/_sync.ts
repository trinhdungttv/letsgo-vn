// Engine reconcile 2 chieu work_tasks <-> Google Tasks cho 1 user.
// Nguyen tac: so updated_at (web) va .updated (Google) voi moc luu trong google_task_links;
...
export interface SyncSummary {
  pushedCreated: number;   // web -> Google: task moi
  pushedUpdated: number;   // web -> Google: cap nhat
...
export async function reconcile(conn: GoogleConnection): Promise<SyncSummary> {
  ...
  const [webTasks, links, gTasks] = await Promise.all([
    ...
    sbSelect<LinkRow>('google_task_links', `user_id=eq.${conn.user_id}&select=id,work_task_id,google_task_id,web_updated_at,google_updated_at`),
  ...
  // ── 2. Task web chua co link -> tao ben Google ──
