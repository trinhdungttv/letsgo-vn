...
- Google → Web: kéo về khi mở trang Bàn làm việc + mỗi 5 phút + nút **Đồng bộ ngay**
...
Chạy 2 file trong Supabase SQL Editor (theo thứ tự):
...
5. Sửa/tick hoàn thành task trong Google Tasks → bấm **Đồng bộ ngay** (hoặc đợi ≤5 phút) → web + Calendar cập nhật.

⚠️ User đã kết nối TRƯỚC khi nâng cấp Calendar cần bấm **Ngắt kết nối** rồi **Kết nối Google** lại
(refresh token cũ chưa có quyền lịch).
...
- Client: `src/lib/googleSync.ts` (gọi API, debounce 2.5s) + `src/components/workspace/GoogleSyncCard.tsx` (UI).
